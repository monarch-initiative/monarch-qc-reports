"""Module for qc difference functions."""
...
def validate_diff_args(f):
    ...
    def wrapped_diff(*args, **kwargs):
        """
        Wrap a function with argument validation.
        ...
        if (
            ...
            or any(key not in flags for key in ("show_all", "change"))
...
@validate_diff_args
def diff_type(
    a: Union[List, Dict, str, int, None], b: Union[List, Dict, str, int, None], flags: Dict
...
def diff_list(a: Union[List, None], b: Union[List, None], flags: Dict) -> List:
    ...
    a_as_keys = dict(zip(a, a, strict=True))
    b_as_keys = dict(zip(b, b, strict=True))
